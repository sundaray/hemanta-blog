...
**The core benefit of `Record` is it keeps related values and types synchronized.**
...
Let's see what happens when we try to break this contract. Imagine a new requirement comes in for a `guest` role. We update the `UserRole` type but forget to update our object:

```ts title="TypeScript" {/guest/}
...
TypeScript will immediately throw an error on the `roleDescriptions` object:

Error Message

This error tells the developer, "Hey, your source of truth (`UserRole`) changed. You must update this object to match it." It forces synchronization and prevents the bug before it ever happens.
