...
Imagine a new requirement comes in for a `guest` role. We update the `UserRole` type but forget to update our object. TypeScript will immediately throw an error on the `roleDescriptions` object:
...
This error tells, "Hey, your source of truth (`UserRole`) changed. You must update this object to match it." It forces synchronization and prevents the bug before it ever happens.
